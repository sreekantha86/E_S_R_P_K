...
using prms.web.Models;
using prms.web.Helpers;
using System.Collections.Generic;
using System.Web.Security;
using prms.web.App_Start;
...
namespace prms.web.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            string encryptedPassword = Helpers.Cryptography.GetSHA1String(model.Password);
            ems.domain.User user = _repo.ValidateLogin(model.UserName, encryptedPassword);
            //string res = "No";

            //int userkey = 0;
            //NcjDomain.User user = (new UserRepository().ValidateLogin(model.UserName, encryptedPassword));

            if (user == null || user.usrUserName == null)
            {
        ...
        private void SignIn(User user, bool isPersistent, HttpCookieCollection cookiecollection)
        {
            var userData = String.Format("{0}|{1}|{2}|{3}",
                user.usrId, user.usrUserName, user.usrGUID, user.usrEmployeeName);
            ...
            userCookie.Values.Add("UserId", user.usrId.ToString());
            userCookie.Values.Add("UserName", user.usrUserName.ToString());
            userCookie.Values.Add("GUID", user.usrGUID.ToString());
            userCookie.Values.Add("usrEmployeeName", user.usrEmployeeName.ToString());
